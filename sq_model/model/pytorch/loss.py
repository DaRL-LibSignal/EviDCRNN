...
def mis(y_pred, y_true):
    ...
    median = 0.5
    mask1 = torch.where(median-d>0,median-d,torch.zeros(d.shape).cuda()).cuda()
    median_prediction = torch.sum(mask1*b,3) + gamma

    low = 0.025
    mask2 = torch.where(low-d>0,low-d,torch.zeros(d.shape).cuda()).cuda()
    low_prediction = torch.sum(mask2*b,3) + gamma

    high = 0.975
    mask3 = torch.where(high-d>0,high-d,torch.zeros(d.shape).cuda()).cuda()
    high_prediction = torch.sum(mask3*b,3) + gamma
