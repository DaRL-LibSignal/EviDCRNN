...
from model.pytorch.loss import mae_loss,rmse_loss
...
class DCRNNSupervisor:
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                losses_rmse.append(self.compute_rmse(y,output).item())

                y_truths.append(y.cpu())
                y_preds.append(output.cpu())

            mean_loss = np.mean(losses)
            rmse_loss = np.mean(losses_rmse)
            ...
            return mean_loss,rmse_loss, {'prediction': y_preds_scaled, 'truth': y_truths_scaled}
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            end_time = time.time()
            val_loss,val_rmse, _ = self.evaluate(dataset='val', batches_seen=batches_seen)
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = 'Epoch [{}/{}] ({}) train_time: {:.4f}, val_mae: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           end_time - start_time, val_loss, lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)

            if (epoch_num % test_every_n_epochs) == test_every_n_epochs - 1:
                test_loss, test_rmse, _ = self.evaluate(dataset='test', batches_seen=batches_seen)
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, test_mae: {:.4f},test_rmse: {:.4f},  lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), test_loss, test_rmse,lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)
